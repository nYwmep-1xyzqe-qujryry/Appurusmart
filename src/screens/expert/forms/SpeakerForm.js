...
const SpeakerForm = ({ navigation }) => {
  ...
  const { items, loading, saving, create, update, remove } =
    useResource("/lecturers");
  const [editingItem, setEditingItem] = useState(null);
  const [form, setForm] = useState(INIT_FORM);

  // API ใช้ field `name` แต่ UI ใช้ `title` → normalize ตอนเปิด edit
  ...
  const handleSave = () => submitOnce(async () => {
    if (!form.year || !form.title.trim()) {
      Alert.alert(
        ...
        t("research.speaker.validation"),
      );
      return;
    }
    const payload = { year: String(form.year), name: form.title.trim() };
    try {
      editingItem
      ...
      >
        ...
        >
          <View className="w-11 h-11 rounded-xl bg-[#e6f4ef] items-center justify-center mr-3">
            <Ionicons name="mic" size={22} color="#07865F" />
          </View>
          <View className="flex-1">
            <Text className="text-[12px] font-bold text-[#5F7069] uppercase" style={{ letterSpacing: 0 }}>
              {t("research.award.heroEyebrow")}
            </Text>
            <Text className="text-[19px] font-black text-[#33483f] mt-[2px]">
              {t("research.speaker.heroTitle")}
            </Text>
          </View>
          ...
          <View className="flex-row items-center justify-between border-b border-[#eef1f4] pb-3 px-4 mb-2">
            <View className="flex-row items-center">
              ...
              <Text className="text-[17px] font-black text-[#33483f] ml-2">
                {editingItem
                  ? t("research.speaker.editForm")
                  : t("research.speaker.addForm")}
              </Text>
            </View>
            {editingItem && (
