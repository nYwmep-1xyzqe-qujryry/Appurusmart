...
export default function ArticleList({ navigation, route }) {
  ...
  const handleDelete = (item) => {
    confirm({
      ...
      message: te("article.deleteConfirm"),
      icon: "trash-outline",
      onConfirm: async () => {
        try {
      ...
      <ScrollView ref={scrollRef} contentContainerStyle={{ padding: 14, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <View className="bg-white rounded-2xl p-3 mb-4 flex-row items-center" style={{ elevation: 1 }}>
          ...
          <TextInput
            ...
            accessibilityLabel={te("article.searchPlaceholder")}
          />
        ...
        <View className="bg-white border border-[#eef1f4] rounded-2xl overflow-hidden mb-4" style={{ elevation: 1 }}>
          <View className="flex-row items-center bg-[#07865F] px-[14px] py-[11px]">
            ...
            <Text className="text-white text-[14px] font-extrabold flex-[3]">{te("article.tableTitle")}</Text>
            <Text className="text-white text-[14px] font-extrabold flex-1 text-center">{te("common.file")}</Text>
          ...
          {loading || sessionLoading ? (
            ...
            <View className="items-center py-9">
              ...
              <Text className="text-[15px] font-bold text-[#1f2a2e] mt-[10px]">{te("article.emptyTitle")}</Text>
              <Text className="text-[13px] text-[#5F7069] mt-1">{te("article.emptyHint")}</Text>
            </View>
          ) : (
            items.map((item, index) => {
              ...
              <View key={item.id} className="flex-row items-center px-[14px] py-3 border-t border-[#eef1f4]">
                ...
                <TouchableOpacity
                  ...
                  onPress={() => navigation.navigate("ResearchDocumentDetail", { type: "paper", item })}
                  activeOpacity={0.7}
                  accessibilityRole="button"
                  accessibilityLabel={`${te("common.viewDetails")} ${item.title_th || ""}`}
                >
                  <Text className="text-[14px] font-semibold text-[#1f2a2e]" numberOfLines={2}>{item.title_th || "-"}</Text>
                  <Text className="text-[12px] text-[#5F7069] mt-[2px]">{item.source || "-"}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                <View className="flex-1 flex-row gap-[6px] justify-center">
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate("ArticleForm", { item })}
                  >
